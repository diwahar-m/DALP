    ...
    (
        ...
        cy.url().then(() => {
            ...
            if (!forProject) {
                ...
                cy.get('.cvat-project-search-field')
                    .first()
                    .within(() => {
...
Cypress.Commands.add(
    'uploadAnnotations',
    (format, filePath, confirmModalClassName, sourceStorage = null, useDefaultLocation = true) => {
        ...
        if (sourceStorage && sourceStorage.cloudStorageId) {
            cy.get('.cvat-modal-import-dataset').find('.cvat-modal-import-filename-input').type(filePath);
        } else {
...
Cypress.Commands.add('setJobState', (choice) => {
    ...
    cy.get('.cvat-annotation-menu-job-state-submenu')
        .should('not.have.class', 'ant-zoom-big')
        .within(() => {
...
Cypress.Commands.add('exportTask', ({ type, format, archiveCustomName }) => {
...
Cypress.Commands.add(
    'exportJob',
    ({ type, format, archiveCustomName, targetStorage = null, useDefaultLocation = true }) => {
...
Cypress.Commands.add('shapeRotate', (shape, expectedRotateDeg, pressShift = false) => {
    cy.get(shape).trigger('mousemove').trigger('mouseover').should('have.class', 'cvat_canvas_shape_activated');
    cy.get('.svg_select_points_rot').then(($el) => {
        ...
        cy.get('#root').trigger('mousemove', x, y).trigger('mouseenter', x, y);
