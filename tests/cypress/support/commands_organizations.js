...
Cypress.Commands.add('createOrganization', (organizationParams) => {
    ...
    cy.get('.cvat-header-menu')
        ...
        .trigger('mouseover');
    cy.get('.cvat-header-menu-create-organization').should('be.visible').click();
    cy.url().should('contain', '/organizations/create');
    cy.get('.cvat-create-organization-form')
        .should('be.visible')
        .within(() => {
...
Cypress.Commands.add('activateOrganization', (organizationShortName) => {
    ...
    cy.get('.ant-dropdown')
        ...
        .filter(':contains("Organization")')
        .trigger('mouseover');
    cy.contains('.cvat-header-menu-organization-item', organizationShortName).should('be.visible').click();
    cy.get('.cvat-header-menu-user-dropdown').should('be.visible');
    cy.get('.cvat-header-menu-user-dropdown-organization').should('exist').and('have.text', organizationShortName);
});
...
Cypress.Commands.add('openOrganization', (organizationShortName) => {
    ...
    cy.get('.ant-dropdown')
        ...
        .filter(':contains("Organization")')
        .trigger('mouseover');
    cy.get('.cvat-header-menu-active-organization-item').should('have.text', organizationShortName);
    cy.get('.cvat-header-menu-open-organization').should('be.visible').click();
...
Cypress.Commands.add('checkOrganizationMembers', (expectedMembersCount, expectedOrganizationMembers) => {
    ...
    cy.get('.cvat-organization-member-item-username')
        .each((el) => {
...
Cypress.Commands.add('inviteMembersToOrganization', (members) => {
    ...
    for (const el of members) {
        ...
        cy.get('.cvat-organization-invitation-field-email').find('[aria-label="check-circle"]').should('exist');
    ...
    cy.get('.cvat-organization-invitation-modal').contains('button', 'OK').click();
});
...
Cypress.Commands.add('removeMemberFromOrganization', (username) => {
    cy.contains('.cvat-organization-member-item-username', username)
        ...
        .click();
    cy.get('.cvat-modal-organization-member-remove').contains('button', 'Yes, remove').click();
});
