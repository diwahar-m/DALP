...
const { dummyGoogleStorage, dummyAzureContainer, dummyAWSBucket } = require('../../support/dummy-data');
...
context('Dummy cloud storages.', { browser: '!firefox' }, () => {
    ...
    function testCSValues({ status, id, displayName, provider, description }) {
        cy.get('.cvat-cloud-storage-item').then((csItem) => {
            cy.get(csItem).should('have.length', 1).should('contain', `Status: ${status}`);
            if (description) {
                cy.get('.cvat-cloud-storage-description-icon').should('have.length', 1).trigger('mouseover');
                cy.get('[role="tooltip"]').should('be.visible').and('have.text', description);
            }
            if (displayName) {
    ...
    function testCSSetStatusPreview(id, status, image) {
        ...
        cy.intercept('GET', `api/cloudstorages/${id}/preview?**`, { fixture: `${imageFolder}/${image}` }).as(
            'csPreview',
        );
    ...
    describe(`Testing case "${caseId}"`, () => {
        it('Create dummy Google Cloud Storage and check fields.', () => {
            ...
            cy.get('#display_name').should('be.visible').and('have.value', 'Demo GCS');
            cy.get('#description').should('be.visible').and('have.value', 'It is first google cloud storage');
            cy.get('#provider_type').should('be.disabled');
            cy.get('.cvat-cloud-storage-select-provider').should('be.visible').and('have.text', 'Google Cloud Storage');
            cy.get('#resource').should('be.disabled').and('have.attr', 'value', 'gcsbucket');
            cy.get('[title="Key file"]').should('be.visible');
            cy.get('#key_file').should('be.visible');
            cy.get('[title="fakeKey.json"]').should('be.visible');
            cy.get('#prefix').should('be.visible').and('have.value', '');
            cy.get('#project_id').should('be.visible').and('have.value', '');
            cy.get('#location').should('exist');
            cy.get('[placeholder="manifest.jsonl"]').should('have.attr', 'value', 'manifest.jsonl');
        });
        ...
        it('Create dummy Azure Blob Container and check fields.', () => {
            ...
            cy.get('#display_name').should('be.visible').and('have.value', 'Demonstration container');
            cy.get('#description').should('be.visible').and('have.value', 'It is first container');
            cy.get('#provider_type').should('be.disabled');
            cy.get('.cvat-cloud-storage-select-provider').should('be.visible').and('have.text', 'Azure Blob Container');
            cy.get('#resource').should('be.disabled').should('be.visible').and('have.attr', 'value', 'container');
            cy.get('[title="Account name and SAS token"]').should('be.visible');
            cy.get('#account_name').should('be.visible').and('not.have.value', '');
            cy.get('#SAS_token').should('be.visible').and('not.have.value', '');
            cy.get('[placeholder="manifest.jsonl"]').should('have.attr', 'value', 'manifest.jsonl');
        });
        ...
        it('Create dummy AWS S3 and check fields.', () => {
            ...
            cy.get('#display_name').should('be.visible').and('have.value', 'Demonstration bucket');
            cy.get('#description').should('be.visible').and('have.value', 'It is first bucket');
            cy.get('#provider_type').should('be.disabled');
            cy.get('.cvat-cloud-storage-select-provider').should('be.visible').and('have.text', 'AWS S3');
            cy.get('#resource').should('be.disabled').should('be.visible').and('have.attr', 'value', 'bucket');
            cy.get('[title="Key id and secret access key pair"]').should('be.visible');
            cy.get('#key').should('be.visible').and('not.have.value', '');
            cy.get('#secret_key').should('be.visible').and('not.have.value', '');
