...
from rest_framework import views, serializers , status # status added
...
from .serializers import RegisterSerializerEx
...
class RegisterViewEx(RegisterView):
    ...
    #     user = serializer.save()  # saving the user details in the user table
    #     category_name = self.request.data.get('category')  # getting category value from the request
    #     if category_name:
    #         category = get_object_or_404(Category, category=category_name) # saving category value in the category table database
    #         user.category = category
    #         user.save()
    #         return Response('Category saved')
    #     return Response('Category is not in request')
    ...
    #         category = Category.objects.create(**other_data)
    ...
    # def perform_create(self, serializer):
    #     user = serializer.save()  # Save user data
    ...
    def get_response_data(self, user):
        ...
        if allauth_settings.EMAIL_VERIFICATION != \
            ...
            data['category'] = user.category # new
        return data
